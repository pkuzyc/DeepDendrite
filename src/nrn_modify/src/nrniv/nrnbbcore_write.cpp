      ...
      if (ps->gid_ >= 0) {
        ...
        if (ps->osrc_) {
          ...
          if (nrn_is_artificial_[type]) {
            ...
            cgs[ith].netcon_srcgid[i] = -(type + 100*ix);
          }else{
    ...
    for (size_t j = 0; j < mla.size(); ++j) {
      ...
      if (nrn_has_net_event(type)) {
        ...
        for (int j=0; j < ml->nodecount; ++j) {
          ...
          int agid = -1;
          if (nrn_is_artificial_[type]) {
            agid = -(type + 100*nrncore_art2index(pnt->prop->param));
            if (j < 20)
                printf("mpiid:%d j:%d type:%d agid:%d ix:%d\n", nrnmpi_myid, j, type, agid, nrncore_art2index(pnt->prop->param));
