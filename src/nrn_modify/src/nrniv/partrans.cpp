...
static void nrnbbcore_gap_info() {
  NonVSrcUpdateInfo::iterator it;
  for (it = non_vsrc_update_info_.begin(); it != non_vsrc_update_info_.end(); it++)
  {
      printf("non_vsrc: %d\n", it->first);
  }
  ...
  // all the same type
  int count_type24 = 0, count_type25 = 0;
  printf("all count:%d\n", targets_->count());
  for (int i = 0; i < targets_->count(); ++i)
  {
      if (target_pntlist_->item(i)->prop->type == 24)
          count_type24++;
      if (target_pntlist_->item(i)->prop->type == 25)
          count_type25++;
  }
  printf("type24:%d type25:%d\n", count_type24, count_type25);
  for (int i=0; i < targets_->count(); ++i) {
    //printf("i:%d type:%d\n", i, target_pntlist_->item(i)->prop->type);
