...
#define t _nt->_t
#define dt _nt->_dt
#define index _p[0*_STRIDE]
#define etime _p[1*_STRIDE]
...
static inline int element(_threadargsproto_);
static inline int play(_threadargsproto_);
...
#error AoSoA not implemented.
...
#endif
 {
   if ( _lflag  == 1.0 ) {
     ...
     if ( index > 0.0 ) {
       artcell_net_send ( _tqitem, _weight_index, _pnt, t +  etime - t , 1.0 ) ;
       }
     }
   } 
#if NET_RECEIVE_BUFFERING
...
static inline void initmodel(_threadargsproto_) {
  int _i; double _save;{
 {
   ...
   element ( _threadargs_ ) ;
   if ( index > 0.0 ) {
     artcell_net_send ( _tqitem, -1, (Point_process*) _nt->_vdata[_ppvar[1*_STRIDE]], t +  etime - t , 1.0 ) ;
     }
   }
