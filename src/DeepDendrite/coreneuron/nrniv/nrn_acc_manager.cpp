...
void setup_nrnthreads_on_device(NrnThread* threads, int nthreads) {
    ...
    for (i = 0; i < nthreads; i++) {
        ...
        if (need_record && nt->nrec_param > 0)
        {
            int *d_rec_mech_types, *d_rec_param_index;
            double **d_rec_ptrs;
            float *d_rec_times;
            float* d_rec_vals;
            bool* d_is_rec_v;
            int nparam = nt->nrec_param;
            int rec_len = nt->rec_len;
            
            d_rec_ptrs = (double**)acc_copyin(nt->rec_ptrs, sizeof(double*) * nparam);
            acc_memcpy_to_device(&(d_nt->rec_ptrs), &d_rec_ptrs, sizeof(double**));

            d_rec_vals = (float*)acc_copyin(nt->rec_vals, sizeof(float) * nparam * rec_len);
            acc_memcpy_to_device(&(d_nt->rec_vals), &d_rec_vals, sizeof(float*));

            d_rec_mech_types = (int*)acc_copyin(nt->rec_mech_types, sizeof(int) * nparam);
            acc_memcpy_to_device(&(d_nt->rec_mech_types), &d_rec_mech_types, sizeof(int*));

            d_rec_param_index = (int*)acc_copyin(nt->rec_param_index, sizeof(int) * nparam);
            acc_memcpy_to_device(&(d_nt->rec_param_index), &d_rec_param_index, sizeof(int*));

            d_is_rec_v = (bool*)acc_copyin(nt->is_rec_v, sizeof(bool) * nparam);
            acc_memcpy_to_device(&(d_nt->is_rec_v), &d_is_rec_v, sizeof(bool*));

            if (nt->rec_times)
            {
                d_rec_times = (float*)acc_copyin(nt->rec_times, sizeof(float) * rec_len);
                acc_memcpy_to_device(&(d_nt->rec_times), &d_rec_times, sizeof(float*));
            }
            ...
            {
                //printf("iparam:%d\n", iparam);
                int mech_type = nt->rec_mech_types[iparam];
                int ix = nt->rec_param_index[iparam];
                if (nt->is_rec_v[iparam])
                {
                    nt->rec_ptrs[iparam] = nt->_actual_v + ix;
                }
                else
                {
                    nt->rec_ptrs[iparam] = nt->_ml_list[mech_type]->data + ix;
                }
            }
        ...
        if (has_non_v_gap)
        {
            ...
            int *d_gap_src_mech_types, *d_gap_tgt_mech_types; // gap junction for non-voltage parameters
            int *d_gap_src_param_idx, *d_gap_tgt_param_idx;
            double **d_gap_src_param_ptrs, **d_gap_tgt_param_ptrs;
            bool *d_is_rec_v;

            d_gap_src_mech_types = (int*)acc_copyin(nt->gap_src_mech_types, sizeof(int) * nparam);
            acc_memcpy_to_device(&(d_nt->gap_src_mech_types), &d_gap_src_mech_types, sizeof(int*));

            d_gap_tgt_mech_types = (int*)acc_copyin(nt->gap_tgt_mech_types, sizeof(int) * nparam);
            acc_memcpy_to_device(&(d_nt->gap_tgt_mech_types), &d_gap_tgt_mech_types, sizeof(int*));

            d_gap_src_param_idx = (int*)acc_copyin(nt->gap_src_param_idx, sizeof(int) * nparam);
            acc_memcpy_to_device(&(d_nt->gap_src_param_idx), &d_gap_src_param_idx, sizeof(int*));

            d_gap_tgt_param_idx = (int*)acc_copyin(nt->gap_tgt_param_idx, sizeof(int) * nparam);
            acc_memcpy_to_device(&(d_nt->gap_tgt_param_idx), &d_gap_tgt_param_idx, sizeof(int*));

            d_gap_src_param_ptrs = (double**)acc_copyin(nt->gap_src_param_ptrs, sizeof(double*) * nparam);
            acc_memcpy_to_device(&(d_nt->gap_src_param_ptrs), &d_gap_src_param_ptrs, sizeof(double**));

            d_gap_tgt_param_ptrs = (double**)acc_copyin(nt->gap_tgt_param_ptrs, sizeof(double*) * nparam);
            acc_memcpy_to_device(&(d_nt->gap_tgt_param_ptrs), &d_gap_tgt_param_ptrs, sizeof(double**));
            d_is_rec_v = (bool*)acc_copyin(nt->gap_src_is_v, sizeof(bool) * nparam);
            acc_memcpy_to_device(&(d_nt->gap_src_is_v), &d_is_rec_v, sizeof(bool*));
            d_is_rec_v = (bool*)acc_copyin(nt->gap_tgt_is_v, sizeof(bool) * nparam);
            acc_memcpy_to_device(&(d_nt->gap_tgt_is_v), &d_is_rec_v, sizeof(bool*));

            #pragma acc parallel loop present(nt[0:1])
            for (int iparam = 0; iparam < nparam; iparam++)
            {
                int mech_type, ix;
                mech_type = nt->gap_src_mech_types[iparam];
                ix = nt->gap_src_param_idx[iparam];
                if (nt->gap_src_is_v[iparam])
                {
                    nt->gap_src_param_ptrs[iparam] = nt->_actual_v + ix;
                }
                else
                {
                    nt->gap_src_param_ptrs[iparam] = nt->_ml_list[mech_type]->data + ix;
                }
                mech_type = nt->gap_tgt_mech_types[iparam];
                ix = nt->gap_tgt_param_idx[iparam];
                if (nt->gap_tgt_is_v[iparam])
                {
                    nt->gap_tgt_param_ptrs[iparam] = nt->_actual_v + ix;
                }
                else
                {
                    nt->gap_tgt_param_ptrs[iparam] = nt->_ml_list[mech_type]->data + ix;
                }
            }
        }

        int *d_idx = NULL, *d_mech_types = NULL;
        double **d_ptr = NULL;
        int len;
        int mech_type, ix;
        if (is_training || is_testing)
        {
            ...
            len = nt->stim_len;
            d_idx = (int*)acc_copyin(nt->stim_idx, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->stim_idx), &d_idx, sizeof(int*));

            d_mech_types = (int*)acc_copyin(nt->stim_mech_types, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->stim_mech_types), &d_mech_types, sizeof(int*));

            d_ptr = (double**)acc_copyin(nt->stim_ptrs, sizeof(double*) * len);
            acc_memcpy_to_device(&(d_nt->stim_ptrs), &d_ptr, sizeof(double**));

            #pragma acc parallel loop present(nt[0:1])
            for (int i = 0; i < nt->stim_len; i++)
            {
                mech_type = nt->stim_mech_types[i];
                ix = nt->stim_idx[i];
                nt->stim_ptrs[i] = nt->_ml_list[mech_type]->data + ix;
            }

            double *d_imgs = NULL;
            len = nt->nimg * nt->img_size;
            d_imgs = (double*)acc_copyin(nt->imgs, sizeof(double) * len);
            acc_memcpy_to_device(&(d_nt->imgs), &d_imgs, sizeof(double*));

            int *d_labels = NULL;
            len = nt->nimg;
            d_labels = (int*)acc_copyin(nt->labels, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->labels), &d_labels, sizeof(int*));
        ...
        if (is_training)
        {
            ...
            len = nt->nweights;
            d_idx = (int*)acc_copyin(nt->w_idx, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->w_idx), &d_idx, sizeof(int*));

            d_mech_types = (int*)acc_copyin(nt->w_mech_types, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->w_mech_types), &d_mech_types, sizeof(int*));

            d_ptr = (double**)acc_copyin(nt->w_ptrs, sizeof(double*) * len);
            acc_memcpy_to_device(&(d_nt->w_ptrs), &d_ptr, sizeof(double**));

            #pragma acc parallel loop present(nt[0:1])
            for (int i = 0; i < nt->nweights; i++)
            {
                mech_type = nt->w_mech_types[i];
                ix = nt->w_idx[i];
                nt->w_ptrs[i] = nt->_ml_list[mech_type]->data + ix;
            }

            d_idx = (int*)acc_copyin(nt->dw_idx, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->dw_idx), &d_idx, sizeof(int*));

            d_mech_types = (int*)acc_copyin(nt->dw_mech_types, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->dw_mech_types), &d_mech_types, sizeof(int*));

            d_ptr = (double**)acc_copyin(nt->dw_ptrs, sizeof(double*) * len);
            acc_memcpy_to_device(&(d_nt->dw_ptrs), &d_ptr, sizeof(double**));
            
            #pragma acc parallel loop present(nt[0:1])
            for (int i = 0; i < nt->nweights; i++)
            {
                mech_type = nt->dw_mech_types[i];
                ix = nt->dw_idx[i];
                nt->dw_ptrs[i] = nt->_ml_list[mech_type]->data + ix;
            }

            len = nt->in2hd_len;
            d_idx = (int*)acc_copyin(nt->in2hd_idx, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->in2hd_idx), &d_idx, sizeof(int*));

            d_mech_types = (int*)acc_copyin(nt->in2hd_mech_types, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->in2hd_mech_types), &d_mech_types, sizeof(int*));

            d_ptr = (double**)acc_copyin(nt->in2hd_ptrs, sizeof(double*) * len);
            acc_memcpy_to_device(&(d_nt->in2hd_ptrs), &d_ptr, sizeof(double**));

            #pragma acc parallel loop present(nt[0:1])
            for (int i = 0; i < nt->in2hd_len; i++)
            {
                mech_type = nt->in2hd_mech_types[i];
                ix = nt->in2hd_idx[i];
                nt->in2hd_ptrs[i] = nt->_ml_list[mech_type]->data + ix;
            }

            len = nt->hd2out_len;
            d_idx = (int*)acc_copyin(nt->hd2out_idx, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->hd2out_idx), &d_idx, sizeof(int*));

            d_mech_types = (int*)acc_copyin(nt->hd2out_mech_types, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->hd2out_mech_types), &d_mech_types, sizeof(int*));

            d_ptr = (double**)acc_copyin(nt->hd2out_ptrs, sizeof(double*) * len);
            acc_memcpy_to_device(&(d_nt->hd2out_ptrs), &d_ptr, sizeof(double**));
            
            #pragma acc parallel loop present(nt[0:1])
            for (int i = 0; i < nt->hd2out_len; i++)
            {
                mech_type = nt->hd2out_mech_types[i];
                ix = nt->hd2out_idx[i];
                nt->hd2out_ptrs[i] = nt->_ml_list[mech_type]->data + ix;
            }

            len = nt->in2hd_g_len;
            d_idx = (int*)acc_copyin(nt->in2hd_g_idx, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->in2hd_g_idx), &d_idx, sizeof(int*));

            d_mech_types = (int*)acc_copyin(nt->in2hd_g_mech_types, sizeof(int) * len);
            acc_memcpy_to_device(&(d_nt->in2hd_g_mech_types), &d_mech_types, sizeof(int*));

            d_ptr = (double**)acc_copyin(nt->in2hd_g_ptrs, sizeof(double*) * len);
            acc_memcpy_to_device(&(d_nt->in2hd_g_ptrs), &d_ptr, sizeof(double**));

            #pragma acc parallel loop present(nt[0:1])
            for (int i = 0; i < nt->in2hd_g_len; i++)
            {
                mech_type = nt->in2hd_g_mech_types[i];
                ix = nt->in2hd_g_idx[i];
                nt->in2hd_g_ptrs[i] = nt->_ml_list[mech_type]->data + ix;
            }
        }

        if (nt->resetv_each_node != NULL)
        {
            double *d_vals = NULL;
            d_vals = (double*)acc_copyin(nt->resetv_each_node, sizeof(double) * nt->end);
            acc_memcpy_to_device(&(d_nt->resetv_each_node), &d_vals, sizeof(double*));
        }

        //if (is_testing)
        //{
        //    float *d_weights_buffer = NULL;
        //    d_weights_buffer = (float*)acc_copyin(nt->weights_buffer, sizeof(float) * nt->nweights);
        //    acc_memcpy_to_device(&(d_nt->weights_buffer), &d_weights_buffer, sizeof(float*));
        //    #pragma acc parallel loop present(nt[0:1])
        //    for (int i = 0; i < nt->nweights; i++)
        //    {
        ...
        if (nt->_permute) {
            if (use_interleave_permute == 1) {
                ...
                acc_memcpy_to_device(&(d_info->cellsize), &d_ptr, sizeof(int*));
            } else if (use_interleave_permute == 3) {
                /* todo: not necessary to setup pointers, just copy it */
                InterleaveInfo* info = interleave_info + i;
                InterleaveInfo* d_info = (InterleaveInfo*)acc_copyin(info, sizeof(InterleaveInfo));
                int* d_ptr = NULL;

                int thread_per_cell = info->thread_per_cell;
                printf("nrn_acc_manager.cpp:nthread:%d thread_num:%d norder:%d\n", thread_per_cell, info->threads_num, info->norder);

                d_ptr = (int*)acc_copyin(info->firstnode, sizeof(int) * info->threads_num);
                acc_memcpy_to_device(&(d_info->firstnode), &d_ptr, sizeof(int*));

                d_ptr = (int*)acc_copyin(info->lastnode, sizeof(int) * info->threads_num);
                acc_memcpy_to_device(&(d_info->lastnode), &d_ptr, sizeof(int*));

                d_ptr = (int*)acc_copyin(info->next_node, sizeof(int) * nt->end);
                acc_memcpy_to_device(&(d_info->next_node), &d_ptr, sizeof(int*));

                d_ptr = (int*)acc_copyin(info->prev_node, sizeof(int) * nt->end);
                acc_memcpy_to_device(&(d_info->prev_node), &d_ptr, sizeof(int*));               

                d_ptr = (int*)acc_copyin(info->max_order_per_thread, sizeof(int) * (info->threads_num));
                acc_memcpy_to_device(&(d_info->max_order_per_thread), &d_ptr, sizeof(int*));

                d_ptr = (int*)acc_copyin(info->min_order_per_thread, sizeof(int) * (info->threads_num));
                acc_memcpy_to_device(&(d_info->min_order_per_thread), &d_ptr, sizeof(int*));

                d_ptr = (int*)acc_copyin(info->stride, sizeof(int) * (info->nwarp) * (info->norder + 1));
                acc_memcpy_to_device(&(d_info->stride), &d_ptr, sizeof(int*));

                d_ptr = (int*)acc_copyin(info->map_t2c, sizeof(int) * info->threads_num);
                acc_memcpy_to_device(&(d_info->map_t2c), &d_ptr, sizeof(int*));

            
            } else {
                printf("\n ERROR: only --cell_permute = [123] implemented");
                abort();
            }
        } else {
